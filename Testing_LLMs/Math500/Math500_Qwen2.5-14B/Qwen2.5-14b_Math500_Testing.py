...
def extract_answer(response_text):
    # Prioritize "p - q" extraction
    pq_match = re.search(r'p\s*-\s*q', response_text)
    if pq_match:
        # Check if it's in the answer section or conclusion
        if re.search(r'(?:answer|result|expressed as).*p\s*-\s*q', response_text.lower()) or \
           ('p - q' in response_text.split('\n\n')[-1] or 'p-q' in response_text.split('\n\n')[-1]):
            return "p - q"

    # Look for an answer preceded by "Answer:" or "answer:"
    ...
    # Try to extract coordinates if "p-q" is not found
...
def are_answers_equivalent(answer1, answer2):
    ...
    # Special case for p-q - check before normalization
    if "p-q" in answer1.replace(" ", "").lower() or "p-q" in answer2.replace(" ", "").lower():
        return "p-q" in answer1.replace(" ", "").lower() and "p-q" in answer2.replace(" ", "").lower()
